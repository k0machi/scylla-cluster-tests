...
class Nemesis:  # pylint: disable=too-many-instance-attributes,too-many-public-methods
    ...
    def get_random_timestamp_from_partition(self, ks_cf, pkey, partition_percentage=0.25) -> tuple[int, int]:
        with self.cluster.cql_connection_patient(node=self.target_node) as session:
            partition_size = session.execute(
                f"select count(ck) from {ks_cf} where pk = {pkey}").one().system_count_ck
            fetch_limit = max(math.ceil(partition_size * partition_percentage), 11)
            self.log.debug(
            ...
            partition = session.execute(f"select pk, ck from {ks_cf} where pk = {pkey} limit {fetch_limit}").all()
            delete_mark = partition[-1].ck
            timestamp = session.execute(
                f"select writetime(v) from {ks_cf} where pk = {pkey} and ck = {delete_mark}").one().writetime_v
            if not timestamp:
    ...
    def _verify_using_timestamp_deletions(self, ks_cf: str, verification_queries: list[tuple[int, int, int]]):
        ...
        with self.cluster.cql_connection_patient(self.target_node, connect_timeout=300) as session:
            ...
            for pk, ck, ts in verification_queries:
                result = session.execute(f"SELECT pk, ck FROM {ks_cf} WHERE pk = {pk} AND ck = {ck}").one()
                assert not result, f"USING TIMESTAMP: deletion failed for ({pk}, {ck}), row still exists, timestamp used: {ts}"
                if not mv_not_configured:
                    try:
                        result = session.execute(
                            f"SELECT pk, ck FROM {mv_table_name} WHERE pk = {pk} AND ck = {ck}").one()
                        assert not result, f"USING TIMESTAMP: deletion failed for ({pk}, {ck}), " \
    ...
    def delete_by_range_using_timestamp(self, ks_cf: str):
        ...
        partition_percentage = min(max(random.random(), 0.25), 0.75)
